...
// 2.请不要将本插件用于除维护群聊健康清朗和谐环境以外的其他用途，否则请自行承担该行为产生的一切法律后果，与插件作者无关。
...
independentcfg = [
  ...
  [924017116, true, true, true, true, false, 2],//提瓦特幼稚园
]
...
export async function HpicListener(e) {
  //判定黑白名单
  ...
  if (!e.img || e.isPrivate) return false
  ...
  for (let val of e.message) {
    if (val.type == "image") {
      ...
      client.imageCensorUserDefined(val.url, 'url').then(function (data) {
        // console.log('<imageCensorUserDefined>: ' + JSON.stringify(data));
        // console.log("【色图监听】：data:", data)
        ...
        for (let i = 0; i < independentcfg.length; i++)
          if (independentcfg[i][0] * 1 == e.group_id * 1) {
            // if (independentcfg[i][0]  == e.group_id ) {
        ...
        console.log("【色图监听】：检测到图片\n【色图监听】：群", e.group_name, "的监听策略：存本地", isChiDuShi, "，撤回", iscehui, "，警告", isWarn, "，转发主人", isTellMaster, "，禁言", isMute, "，禁言时长", mutetime, "分钟")
        if (data.conclusionType == 2 || data.conclusionType == 3) {
          console.log("【色图监听】：发现违规图片！");
          if (isChiDuShi) {
            savepic(val.url, data)
          }
          if (iscehui) {
          ...
          if (isTellMaster) {
            ...
            let one = [];
            let msg = [
              "检测到色图：", segment.image(val.url),
              "来自群：", e.group_name,
              "\n发送者：", e.sender.card,]
            for (let i = 0; i < data.data.length; i++) {
              one = [
                "\n---------------------\n类型：", data.data[i].msg,  //注释这一行，并取消下面一行的注释，即可更换推送样式
                // "\n●类型：", data.data[i].msg,
